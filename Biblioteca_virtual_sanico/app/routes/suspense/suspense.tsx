          ...
          {livrosSuspense.map((livro) => (
            <div key={livro.id} className="flex flex-col items-center gap-8 md:flex-row md:items-start">
              ...
              <div className="w-full max-w-xs rounded-2xl border border-gray-800 bg-[#121118] p-4 text-center shadow-2xl">
                ...
                <div className="mb-6 flex h-48 w-full items-center justify-center overflow-hidden rounded bg-[#2a2a2a]">
                  <img src={livro.img} alt={livro.titulo} className="h-full w-full object-cover" />
                </div>
                <h3 className="mb-2 font-serif text-xl tracking-wide text-white">{livro.titulo}</h3>
              </div>

              {/* LADO DIREITO (BALÃO AZUL CLARO + BOTÃO) */}
              <div className="flex flex-1 flex-col justify-between self-stretch pt-2">
                <div className="relative rounded-sm bg-[#dbeafe] p-6 text-black shadow-md">
                  {/* PONTEIRO DO BALÃO DE FALA */}
                  <div className="absolute left-[-10px] top-8 hidden h-0 w-0 border-y-[8px] border-r-[10px] border-y-transparent border-r-[#dbeafe] md:block"></div>
